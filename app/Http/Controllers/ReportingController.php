...
class ReportingController extends Controller
{
    ...
    public function exportCategorySalesCsv(Request $request)
    {
        ...
        $query = OrderItem::query()
            ...
            ->leftJoin('product_batches as item_batches', 'order_items.product_batch_id', '=', 'item_batches.id')
            ->leftJoin('product_barcodes as item_barcodes', 'order_items.product_barcode_id', '=', 'item_barcodes.id')
            ->leftJoin('product_batches as barcode_batches', 'item_barcodes.batch_id', '=', 'barcode_batches.id')
        ...
        // Some legacy/order-edit records can have order_items.unit_price and order_items.total_amount saved as 0.
        // In that case, fall back to the sold item's batch sell_price. Prefer the item batch first,
        // then the scanned barcode's batch. discount_amount is already a line-level value, so do NOT multiply it.
        $lineSubtotalSql = "CASE
            WHEN COALESCE(order_items.unit_price, 0) > 0
                THEN COALESCE(order_items.quantity, 0) * COALESCE(order_items.unit_price, 0)
            WHEN COALESCE(order_items.total_amount, 0) > 0
                THEN COALESCE(order_items.total_amount, 0) + COALESCE(order_items.discount_amount, 0)
            WHEN COALESCE(item_batches.sell_price, 0) > 0
                THEN COALESCE(order_items.quantity, 0) * COALESCE(item_batches.sell_price, 0)
            WHEN COALESCE(barcode_batches.sell_price, 0) > 0
                THEN COALESCE(order_items.quantity, 0) * COALESCE(barcode_batches.sell_price, 0)
            ELSE 0
        END";
        ...
        $categorySales = $query->select(
            ...
            DB::raw("SUM({$lineSubtotalSql}) as subtotal"),
            DB::raw('SUM(COALESCE(order_items.discount_amount, 0)) as total_discount'),
            DB::raw('SUM(COALESCE(order_items.tax_amount, 0)) as total_tax')
        )
