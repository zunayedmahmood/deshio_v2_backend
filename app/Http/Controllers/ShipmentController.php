...
class ShipmentController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        try {
            ...
            // Set COD amount for Pathao. Priority: request override -> order outstanding -> total - paid.
    ...
    public function sendToPathao($shipmentOrId)
    {
        ...
        try {
            ...
            // Derive COD if older pending shipments were created before cod_amount was stored.
    ...
    protected function dispatchPathaoBulkBatch(array $eligibleIds, ?int $storeId, array $immediateFailures = []): \Illuminate\Http\JsonResponse
    {
        ...
        // Keep this conservative unless Pathao confirms a higher rate limit.
        $waveSize = (int) config('services.pathao.bulk_wave_size', 5);
        $waveSize = max(1, min($waveSize, 20));

        foreach ($eligibleIds as $index => $shipmentId) {
            $delaySeconds = intdiv($index, $waveSize);
        ...
        try {
            $retryPayload = DB::transaction(function () use ($batch, $maxRetries) {
                ...
                foreach ($candidateIds as $shipmentId) {
                    ...
                    if (!empty($shipment->pathao_consignment_id)) {
                        $results[$key] = [
                            'success' => true,
                            'message' => 'Already sent to Pathao before retry',
                            'consignment_id' => $shipment->pathao_consignment_id,
                            'processed_at' => now()->toISOString(),
                            'retry_attempts' => (int) ($results[$key]['retry_attempts'] ?? 0),
                        ];
                        continue;
                    }
                    ...
                    $attemptNumber = $retryAttemptsByShipment[$shipmentId];
                    $results[$key] = [
                        'success' => false,
                        'message' => "Queued for Pathao retry attempt {$attemptNumber}",
                        'consignment_id' => null,
                        'processed_at' => now()->toISOString(),
                ...
                $lockedBatch->processed_count = $processedResults->count();
                $lockedBatch->success_count = $processedResults->where('success', true)->count();
                $lockedBatch->failed_count = $processedResults->where('success', false)->count();
            ...
            $retryShipmentIds = $retryPayload['shipment_ids'];

            foreach ($retryShipmentIds as $index => $shipmentId) {
                $delaySeconds = intdiv($index, $waveSize);
            ...
            $freshBatch = PathaoBulkBatch::find($batch->id);

            return response()->json([
                'success' => true,
                'message' => count($retryShipmentIds) > 0
                    ? count($retryShipmentIds) . ' failed shipment(s) queued for retry.'
                    : 'No retryable failed shipment found.',
                'data' => [
                    'batch_code' => $batch->batch_code,
                    'queued_count' => count($retryShipmentIds),
                    'shipment_ids' => $retryShipmentIds,
                    'skipped' => $retryPayload['skipped'],
                    'summary' => $freshBatch?->getSummary(),
                    'already_running' => false,
                ],
            ]);
        } catch (\Throwable $e) {
            ...
            return response()->json([
                'success' => false,
                'message' => 'Failed to retry Pathao batch: ' . $e->getMessage(),
            ], 500);
        } finally {
            optional($lock)->release();
        }
