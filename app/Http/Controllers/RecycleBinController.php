...
class RecycleBinController extends Controller
{
        ...
        try {
            ...
            if ($type === 'all' || $type === 'fields') {
                $fields = Field::onlyTrashed()
                    ->when($search, function ($query) use ($search) {
                        $this->whereAnyLike($query, ['title', 'type'], $search);
                    })
                    ->orderBy($sortBy, $sortDirection)
                    ->get()
                    ->map(function ($item) {
                ...
                $items = array_merge($items, $fields->toArray());
            }
        ...
        try {
            $stats = [
                ...
                'by_type' => [
                    ...
                    'fields' => Field::onlyTrashed()->count(),
                ],
    ...
    {
        $request->validate([
            'type' => 'required|string|in:product,category,employee,vendor,store,customer,order,field',
            'id' => 'required|integer',
        ]);
    ...
    public function restoreMultiple(Request $request)
    {
        $request->validate([
            ...
            'items.*.type' => 'required|string|in:product,category,employee,vendor,store,customer,order,field',
    ...
    public function emptyRecycleBin()
    {
        ...
        try {
            ...
            // Fields
            $deletedCount += Field::onlyTrashed()
                ->where('deleted_at', '<=', $deletionDate)
                ->forceDelete();
